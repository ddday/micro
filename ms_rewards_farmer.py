...
PC_USER_AGENT = (
    ...
    "Safari/537.36 Edg/86.0.622.63"
)
MOBILE_USER_AGENT = (
    "Mozilla/5.0 (Linux; Android 10; Pixel 3) "
    "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0. "
    "3945.79 Mobile Safari/537.36"
)
...
def waitUntilVisible(
    browser: WebDriver, by_: Any, selector: str, time_to_wait: int = 10
):
...
def getCCodeLangAndOffset() -> Tuple[Any, Any, Any]:
    nfo: Union[Any] = ipapi.location()
...
def getGoogleTrends(numberOfwords: int) -> List[str]:
    i = 0
    ...
    while len(search_terms) < numberOfwords:
        i += 1
        trends_dict: Dict[str, Union[int, str]] = {
            "hl": LANG,
            "ed": str((date.today() - timedelta(days=i)).strftime("%Y%m%d")),
            "geo": GEO,
            "ns": 15,
        }
...
def getRelatedTerms(word: str) -> List[str]:
    related_terms_url = "https://api.bing.com/osjson.aspx"
    related_terms_dict = {"query": word}
    related_headers = {"User-agent": PC_USER_AGENT}
    try:
        req = requests.get(
            related_terms_url, params=related_terms_dict, headers=related_headers
        )
        return req.json()[1]
    except:
        return []
...
def openCloseWindow(browser: WebDriver, minTime: int = 8, maxTime: int = 17):
    time.sleep(1)
...
def Generic_Quiz(browser: WebDriver, cardNumber: int):
    time.sleep(1)
    browser.switch_to.window(window_name=browser.window_handles[1])
    time.sleep(8)
...
def completeDailySetThisOrThat(browser: WebDriver, cardNumber: int):
    ...
    ThisOrThat(browser, cardNumber)


def ThisOrThat(browser: WebDriver, cardNumber: int):
    time.sleep(1)
    browser.switch_to.window(window_name=browser.window_handles[1])
    time.sleep(8)
    if not waitUntilQuizLoads(browser):
...
def getDashboardData(browser: WebDriver) -> Dict[Any, Any]:
...
def completePunchCard(browser: WebDriver, url: str, childPromotions: Dict[Any, Any]):
    browser.get(url)
...
def completeMorePromotionQuiz(browser: WebDriver, cardNumber: int):
    ...
    Generic_Quiz(browser, cardNumber)
...
def get_accounts() -> List[Any]:
    account_path = f"{os.path.dirname(os.path.abspath(__file__))}" "/accounts.json"
    if not os.path.isfile(account_path):
        with open(account_path, "w") as file_name:
            file_name.write(
                json.dumps(
        ...
        prPurple(
            ...
            [ACCOUNT] Edit with your credentials and save.
            [ACCOUNT] Restart the program when Done.
            """
        )
        input()
        raise FileNotFoundError("Edit your Credentials")
    with open(account_path, "r") as file_name:
        accounts: List[Any] = json.load(file_name)
